...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  return (
    <section className="mt-14 rounded-[1.8rem] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6 sm:p-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="editable-display text-3xl font-semibold text-[var(--tk-text)]">Comments</h2>
          <p className="text-sm text-[var(--tk-muted)]">{all.length} conversation{all.length === 1 ? '' : 's'}</p>
        </div>
      </div>
      ...
      <form onSubmit={submit} className="mt-6 rounded-[1.4rem] border border-[var(--tk-line)] bg-[var(--tk-raised)] p-5">
        <div className="grid gap-3 sm:grid-cols-[220px_minmax(0,1fr)]">
          <input
            ...
            maxLength={60}
            className="h-12 rounded-[1rem] border border-[var(--tk-line)] bg-transparent px-4 text-sm text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
          />
          <textarea
            value={text}
            ...
            rows={3}
            maxLength={1500}
            className="w-full resize-y rounded-[1rem] border border-[var(--tk-line)] bg-transparent px-4 py-3 text-sm leading-6 text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
          />
        </div>
        <div className="mt-4 flex justify-end">
          <button
            type="submit"
            disabled={!text.trim()}
            className="inline-flex items-center gap-2 rounded-[1rem] bg-[var(--tk-accent)] px-6 py-3 text-sm font-bold text-[var(--tk-on-accent)] transition hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div className="mt-6 grid gap-4">
        {all.map((comment) => (
          <div key={comment.id} className="rounded-[1.3rem] border border-[var(--tk-line)] bg-[var(--tk-raised)] p-5">
            <div className="flex items-center gap-3">
