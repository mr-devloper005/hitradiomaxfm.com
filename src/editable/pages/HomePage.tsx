...
export default async function HomePage() {
  const visiblePrimaryTask = SITE_CONFIG.tasks.find((task) => task.enabled && task.key !== 'listing' && task.key !== 'classified')?.key
  const primaryTask = (visiblePrimaryTask || SITE_CONFIG.tasks.find((task) => task.enabled)?.key || 'article') as TaskKey
